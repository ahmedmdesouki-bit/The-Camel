...
def _noop_observe()  -> Dict[str, Any]: return {}
def _noop_theses(o)  -> List[Any]:      return []
def _noop_choose(t)  -> List[Any]:      return t
def _noop_portfolio()-> PortfolioState: return PortfolioState(fund_usd=0, cash_usd=0)
def _noop_execute(a, d) -> None:        return None
def _noop_measure()  -> Dict[str, Any]: return {}
def _noop_learn(m)   -> None:           return None
...
class LoopRunner:
    ...
    def run_once(self) -> RunState:
        if is_halted():
            ...
            return RunState(phase=self.cfg.phase, outcome="halted")

        state = begin_run(self.cfg.dbs.portfolio, self.cfg.phase)
        try:
            self._execute_loop(state)
        except Exception as exc:
            log.exception("Loop crashed")
            finish_run(self.cfg.dbs.portfolio, state, outcome=f"error: {exc}")
        return state

    def _execute_loop(self, state: RunState) -> None:
        db = self.cfg.dbs.portfolio
        ...
        for action in chosen:
            ...
            if decision.allow:
                ...
                log.info("Guardrail blocked %s: %s",
                         getattr(action, "symbol", "?"), decision.reason)
                acted.append({"symbol": getattr(action, "symbol", "?"),
