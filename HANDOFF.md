...
**Status:** Sprints **S1 → S7 complete** · **S8 core** (10 connectors; rest deferred) · **S9–S11 complete (+ S11.5 integration)** · Dashboard v2 + founder alerts · **543 tests green** · 7-database architecture live
...
(S1–S14) · **S8 core done** (10 connectors; rest deferred). **S9 complete** (entity resolver + Regime Engine + event intelligence/`event_reactions` + multi-state AAOIFI Sharia cross-check + peg). **S10** full Edge Proof · **S10.5** loop assembled (Phase-1 blocker closed) · **S11(+S11.5)** strategies + portfolios + learning, wired end-to-end via `loop/driver.py`. **S12** Edge Lab + realistic paper + ⭐ Sandbox (full system on a live feed + virtual money). **The build (S1–S12) is complete and staged at the live-readiness gate. NEXT = S13 micro-live — founder-gated** (machine hardening + an explicit phase-flip with real money).
...
| Test framework | **pytest** | 543 tests, run via the `N:\` virtual drive (see §6) |
...
| **S9** ◑ in progress | Knowledge Graph + Regime Engine | **slices 1–2 done**: (1) entity resolution — `assets` table + `resolve(ticker)` → identity, Sharia status, ETF look-through, latest filing, delisted flag; (2) **Regime Engine** (`trader/regime/`) — feature builder over `macro_observations` + deterministic 10-state classifier (regime + confidence + themes) + `regime_history` audit. **Next:** event intelligence; Sharia cross-check w/ multi-state status (disagreement → freeze new buys) |
| **S8.5** *(founder)* | Real-Time Data Tier | Streaming quotes (Alpaca IEX websocket) + live-news polling (via sanitiser) + real-time monitor/charts + Telegram alerts → **separate** realtime store (EOD bars untouched); live feed monitoring-only unless quorum ≥2; **execution stays EOD** (real-time execution = Phase-2+ decision) |
...
| **S11** | Strategy Registry + **Portfolio Engine** + Learning | Trio: `core_dca`, `quality_momentum`, `etf_regime_rotation` + `dividend_growth` (Sharia quality-income, not capture); **multi-portfolio** (`portfolios` table, `PortfolioManager`, strategy-per-portfolio, portfolio-scoped positions/ledger reconciling to fund); StrategyMixer; DCA guardrails; intraday monitor; 4-tier learning |
| **S12** | Edge Lab + realistic paper + ⭐ Sandbox Mode | Three run modes incl. **`sandbox`** (full system on live data + virtual money — the dress rehearsal that produces the micro-live track record); bias prevention; walk-forward; crisis tests; benchmark hierarchy; two-engine cross-check; delisted handling; Sharia-drag quantified; **No-Edge protocol → DCA**; kill criteria; *(QA-deferred: broker write-atomicity + positions↔ledger reconcile)* |
| **S12.5** *(founder; design now, dormant)* | Research Desk / Analyst Agents | Per-vertical agents (macro/sector/single-name/geopolitics/Sharia/strategy) study→analyze→store **evidence only, never act** (Agent SDK); on-demand by default; token budget; master switch defaults OFF until capital/edge justify |
| **S13** | Micro-Live Readiness | Approval Channel (Telegram one-tap, timeout=veto); LiveBroker (Alpaca) + ManualBroker (Sahm); limit-orders-only; Phase 1 entry ($100–500, human-approved) |
