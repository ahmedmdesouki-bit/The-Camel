...
**Stack:** Python 3.12 (agent + guardrail + loop) · Supabase (Postgres) · TypeScript/HTML (dashboard) · GitHub Actions or Windows Task Scheduler/cron for scheduling · pytest for the guardrail suite.
...
## 10. Machine setup (what you do on the Windows PC)
...
3. **Secrets:** Alpaca key scoped **trade-only, withdrawals OFF**. Enable BitLocker + firewall + auto-updates. Tailscale ACL locked to your devices, MFA everywhere.
4. **Schedule:** Windows Task Scheduler job runs the loop entrypoint post-close; a separate cron runs the quarterly Sharia re-screen.
