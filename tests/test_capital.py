...
def test_valid_request_approved():
    result = Allocator().request(buy(), base_state())
...
def test_off_whitelist_rejected():
    result = Allocator().request(buy(symbol="AAPL"), base_state())
...
def test_over_position_cap_rejected():
    ...
    result = Allocator().request(buy(notional=2001.0), base_state())
    assert not result.approved
...
def test_daily_loss_stop_rejects():
    result = Allocator().request(buy(), base_state(day_pnl_pct=-0.06))
    assert not result.approved
...
def test_custom_constitution_respected():
    ...
    result = alloc.request(buy(notional=600.0), base_state())
