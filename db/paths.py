"""
Noah seven-database path registry.

Phase 0 runs seven SQLite files, each owning one data domain.
Caller constructs NoahDbs.from_dir(base_dir) and passes the right
...
@dataclass
class NoahDbs:
    """Holds paths to all seven Noah databases."""
    ...
    @classmethod
    def from_dir(cls, base_dir: str) -> "NoahDbs":
        """Build paths under base_dir using the canonical file names."""
        base = Path(base_dir)
        return cls(
            market=str(base / "noah_market.db"),
            macro=str(base / "noah_macro.db"),
            fundamentals=str(base / "noah_fundamentals.db"),
            news=str(base / "noah_news.db"),
            sharia=str(base / "noah_sharia.db"),
            portfolio=str(base / "noah_portfolio.db"),
            learning=str(base / "noah_learning.db"),
        )


def init_all(dbs: NoahDbs) -> None:
